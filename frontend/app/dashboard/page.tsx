  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.error("Errore scaricamento dati", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 pb-24 font-sans selection:bg-gym-yellow selection:text-gym-red">
      ...
      <header className="px-6 pt-12 pb-6 flex justify-between items-end bg-white/80 backdrop-blur-md sticky top-0 z-10 border-b border-gray-100">
        <div>
          <p className="text-gray-500 text-sm font-bold uppercase tracking-wider mb-1">
            {activeTab === "home" ? "Bentornato" : "Il tuo"}
          </p>
          <h1 className="text-3xl font-black text-gym-red tracking-tight italic uppercase">
            {activeTab === "home" ? username : "Storico"}
          </h1>
        </div>
      </header>
      ...
      <main className="px-5 space-y-8 mt-6">
              ...
              {loading ? (
                ...
                >
                  ...
                  <div className="relative z-10">
                    <span className="bg-gym-yellow text-gym-red text-xs font-black px-2 py-1 rounded mb-3 inline-block uppercase tracking-wide">In Evidenza</span>
        ...
        {activeTab === "history" && (
          <section>
            {loading ? (
              <div className="text-center py-10 text-gray-400 font-medium">Caricamento storico...</div>
            ) : sessions.length === 0 ? (
              <div className="bg-white rounded-xl p-8 text-center border border-gray-100 shadow-sm">
                <p className="text-gray-500 mb-2 font-medium">Nessuna sessione completata.</p>
                <p className="text-sm text-gym-red font-bold">Inizia ad allenarti!</p>
              </div>
            ) : (
              <div className="space-y-4">
                {sessions.map((sess) => (
                  <div key={sess.id} className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm flex items-center justify-between">
                    <div>
                      <h4 className="font-bold text-gray-900 text-lg line-clamp-1">{sess.workout_title}</h4>
                      <p className="text-gray-500 text-sm">{new Date(sess.created_at).toLocaleDateString()}</p>
                    </div>
                    <div className="text-right">
                      <span className="bg-green-100 text-green-700 font-bold text-xs px-2 py-1 rounded inline-block uppercase tracking-wide mb-1">Completato</span>
                      <p className="text-sm text-gray-900 font-bold font-mono">{formatDuration(sess.duration_seconds)}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <nav className="fixed bottom-0 w-full bg-white border-t border-gray-100 pb-safe pt-2 px-6 flex justify-between items-center z-50 h-20 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
        <button 
          ...
          className={`flex flex-col items-center gap-1 w-16 transition ${activeTab === 'home' ? 'text-gym-red' : 'text-gray-400 hover:text-gray-600'}`}
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>
          <span className="text-[10px] font-black tracking-wide">HOME</span>
        </button>

        <button 
          onClick={() => setActiveTab("history")} 
          className={`flex flex-col items-center gap-1 w-16 transition ${activeTab === 'history' ? 'text-gym-red' : 'text-gray-400 hover:text-gray-600'}`}
        >
